...
export class PostEditComponent implements OnInit {
  ...
  tags = ['Marvel', 'DC', 'Malabares', 'Celulares', 'Breaking Bad', 'Samsung', 'Iphone'];
  ...
  ngOnInit() {
    ...
    this.tagsForm = this.createTagsForm(this.tags);
  }
  ...
  buttonAction() {
    ...
    if(ruta[2] === "create") {
      let newPost = this.service.create('posts', this.postForm.value);
      console.log(newPost);
    }else {
      let id = this.route.snapshot.paramMap.get('id');

      let updatedPost = this.service.updateById('posts', id , this.postForm.value);
